...
class SCD_WCFM_Marketplace_Report_Sales_By_Date extends WC_Admin_Report {
	...
	public function query_report_data() {
		...
		if (!function_exists('scd_wcfmmp_get_commission_meta_sum')) {
			# code...
			include WP_PLUGIN_DIR.'/scd-smart-currency-detector-variant-for-wcfm/includes/scd-wcfm-functions.php';
		}
		...
		foreach( $results as $data ) {
			...
			$data->tax *= $rate;
			$data->shipping_tax_amount *= $rate;
			$data->shipping *= $rate;
			$data->total_commission *=$rate;
			$data->refunded_amount *=$rate;
		...
		if( apply_filters( 'wcfmmmp_gross_sales_respect_setting', true ) ) {
			$gross_sales_amount = (float) scd_wcfmmp_get_commission_meta_sum( $gross_commission_ids, 'gross_total' );
		} else {
			$gross_sales_amount = (float) scd_wcfmmp_get_commission_meta_sum( $gross_commission_ids, 'gross_sales_total' );
		}
		...
		if( $admin_fee_mode ) {
		  ...
		  $net_paid_sales_amount = scd_wcfm_get_gross_sales_by_vendor( $vendor_id, $this->current_range, true );
	...
	public function get_main_chart( $show_legend = 1 ) {
		...
		if( !empty( $results ) ) {
			foreach( $results as $result ) {
				...
				if( apply_filters( 'wcfmmmp_gross_sales_respect_setting', true ) ) {
					$gross_sales = (float) scd_wcfmmp_get_commission_meta_sum( $commission_ids, 'gross_total' );
				} else {
					$gross_sales = (float) scd_wcfmmp_get_commission_meta_sum( $commission_ids, 'gross_sales_total' );
				}
		...
		if( !empty( $results ) ) {
			foreach( $results as $result ) {
				...
				$commission_ids = explode( ",", $result->commission_ids );
				if( apply_filters( 'wcfmmmp_gross_sales_respect_setting', true ) ) {
					$paid_gross_sales = (float) scd_wcfmmp_get_commission_meta_sum( $commission_ids, 'gross_total' );
				} else {
					$paid_gross_sales = (float) scd_wcfmmp_get_commission_meta_sum( $commission_ids, 'gross_sales_total' );
				}
